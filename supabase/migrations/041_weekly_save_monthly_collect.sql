-- Weekly pay-ins within each monthly collection round (e.g. ₦10k × 4 weeks → ₦160k turn money).

alter table public.groups
  add column if not exists pay_ins_per_cycle integer not null default 1
  check (pay_ins_per_cycle >= 1 and pay_ins_per_cycle <= 52);

alter table public.contributions
  add column if not exists installment_number integer not null default 1
  check (installment_number >= 1 and installment_number <= 52);

alter table public.contributions drop constraint if exists contributions_cycle_id_member_id_key;

alter table public.contributions
  add constraint contributions_cycle_member_installment_key
  unique (cycle_id, member_id, installment_number);
