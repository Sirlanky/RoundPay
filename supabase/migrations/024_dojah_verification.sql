-- Dojah phone + email OTP verification for group admins.

alter table public.profiles
  add column if not exists phone_verified_at timestamptz,
  add column if not exists email_verified_at timestamptz,
  add column if not exists phone_otp_reference_id text,
  add column if not exists email_otp_reference_id text;
...
alter table public.profiles
  ...
  check (identity_verification_method is null or identity_verification_method in ('placeholder', 'provider', 'dojah'));

comment on column public.profiles.phone_verified_at is 'Set after Dojah SMS OTP validates this profile phone.';
comment on column public.profiles.email_verified_at is 'Set after Dojah email OTP validates this profile email.';
  ...
  if tg_op = 'UPDATE' and new.identity_status is distinct from old.identity_status then
    if auth.uid() = new.id and new.identity_status = 'verified' then
      if coalesce(current_setting('roundpay.placeholder_kyc', true), '') <> 'on'
         ...
         and coalesce(new.identity_verification_method, '') not in ('placeholder', 'dojah') then
...
begin
  ...
  if p.identity_status = 'verified' and p.identity_verification_method = 'dojah' then
    return p;
  end if;
  ...
  if p.phone_verified_at is null then
    raise exception 'DOJAH_PHONE_NOT_VERIFIED';
  end if;

  if p.email_verified_at is null then
    raise exception 'DOJAH_EMAIL_NOT_VERIFIED';
  end if;

  perform set_config('roundpay.dojah_kyc', 'on', true);

  update public.profiles
  set
    identity_status = 'verified',
    identity_verification_method = 'dojah',
  ...
  perform set_config('roundpay.dojah_kyc', 'off', true);
